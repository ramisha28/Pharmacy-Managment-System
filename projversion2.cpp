/*
    ...
    **Functions:**
    nextSerialNumber Medication
    generateNextSerialNumber() Medication
    Authentication
    Pharmacy
    Pharmacy() Pharmacy
    readAdminsFromFile() Pharmacy

    **Functions:**
    readMedicinesFromFile() Pharmacy
    displayMedicines() const Pharmacy
    editMedicineStock(int) Pharmacy
    deleteMedicineStock(int) Pharmacy
    displayMedicationsForCustomer() const Pharm...
    addAdmin(const string &, const string &) Phar...
    deleteAdmin(const string &) Pharmacy
    editAdmin(const string &, const string &) Phar...
    previewAllAdmins() const Pharmacy


    ##Class:##
    Customer
    addAdmin(Pharmacy &, const string &, const ...
    deleteAdmin(Pharmacy &, const string &) Sup...
    editAdmin(Pharmacy &, const string &, const ...

    **Functions:**

    previewAllAdmins(const Pharmacy &) const S...
    addAdmin(Pharmacy &, const string &, const ...
    deleteAdmin(Pharmacy &, const string &) Sub...
    editAdmin(Pharmacy &, const string &, const ...
    previewAllAdmins(const Pharmacy &) const S...
    nextCustomerNumber Customer
    main()
*/
...
public:
    Medication(const string &name, int quantity, double price, const string &manufacturer)
        : serialNumber(generateNextSerialNumber()), name(name), quantity(quantity), price(price), manufacturer(manufacturer) {}

    int getSerialNumber() const
    {
        return serialNumber;
    }
    // Getter methods for existing members
    ...
    void displayWithSerialNumber() const
    {
...
private:
    static int generateNextSerialNumber();
};

// Definition of the static member outside of the class
int Medication::nextSerialNumber = 1;

// Definition of the static member function outside of the class
int Medication::generateNextSerialNumber()
{
...
public:
    ...
    void editMedicineStock(int serialNumber);
    void deleteMedicineStock(int serialNumber);
...
void Pharmacy::displayMedicines() const
{
    ...
    {
        medication.displayWithSerialNumber();
        cout << "----------------------" << endl;
    }
}

void Pharmacy::editMedicineStock(int serialNumber)
{
    auto it = find_if(medicines.begin(), medicines.end(),
                      [serialNumber](const auto &medication)
                      { return medication.getSerialNumber() == serialNumber; });
    ...
    if (it != medicines.end())
    {
        ...
        cout << "Enter new details for the medicine:\n";
        cout << "Name: ";
        cin >> name;
        cout << "Quantity: ";
        cin >> quantity;
        ...
        it->setPrice(price);
        it->setManufacturer(manufacturer);

        cout << "Medicine stock updated successfully.\n";
    }
    else
    {
        cout << "Medicine with serial number " << serialNumber << " not found.\n";
    }
}

void Pharmacy::deleteMedicineStock(int serialNumber)
{
    auto it = find_if(medicines.begin(), medicines.end(),
                      [serialNumber](const auto &medication)
                      { return medication.getSerialNumber() == serialNumber; });
    ...
    {
        medicines.erase(it);
        cout << "Medicine with serial number " << serialNumber << " deleted successfully.\n";

        // Update the medicines.txt file
        ofstream outFile("medicines.txt");
        if (outFile.is_open())
        {
            for (const auto &medication : medicines)
            {
                outFile << medication.getName() << " "
                        << medication.getQuantity() << " "
                        << medication.getPrice() << " "
                        << medication.getManufacturer() << "\n";
            }
            outFile.close();
            cout << "Medicine details updated in file.\n";
        }
        else
        {
            ...
            do
            {
                ...
                case 2:
                {
                    ...
                    case 3:
                    {
                        ...
                        cout << "Enter the serial number of the medicine to edit: ";
                        cin >> editSerialNumber;
                        pharmacy.editMedicineStock(editSerialNumber);
                        break;
                    }
                    case 4:
                    {
                        ...
                        cout << "Enter the serial number of the medicine to delete: ";
                        cin >> deleteSerialNumber;
                        pharmacy.deleteMedicineStock(deleteSerialNumber);
                        break;
                    }
                    case 5:
                    {
